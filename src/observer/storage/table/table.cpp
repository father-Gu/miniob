...
RC Table::drop(const char *base_dir)
{
  ...
  // 删除表元数据文件
  string meta_file = table_meta_file(base_dir, table_meta_.name());
  rc               = persist_handler.remove_file(meta_file.c_str());
  if (rc != RC::SUCCESS) {
    LOG_ERROR("Failed to remove meta file %s, errmsg=%s", meta_file.c_str(), strerror(errno));
    return rc;
  }
  // 删除表数据文件
  string data_file = table_data_file(base_dir, table_meta_.name());
  rc               = persist_handler.remove_file(data_file.c_str());
  if (rc != RC::SUCCESS) {
  ...
  for (int i = 0; i < table_meta_.index_num(); i++) {
    ...
    rc = persist_handler.remove_file(index_file.c_str());
    if (rc != RC::SUCCESS) {
      LOG_ERROR("Failed to remove index file %s, errmsg=%s", index_file.c_str(), strerror(errno));
      return rc;
    }
  }
...
{
  // 1. 直接调用 engine 的 delete_record (这个在 HeapTableEngine 里已经实现了)
  // 它会负责删除数据和更新索引
  RC rc = engine_->delete_record(old_record);
  if (rc != RC::SUCCESS) {
    LOG_WARN("failed to delete old record during update. rc=%s", strrc(rc));
    return rc;
  }

  // 2. 直接调用 engine 的 insert_record (这个在 HeapTableEngine 里也实现了)
  // 注意：我们需要一个非 const 的 copy
  Record record_to_insert = new_record;
  rc                      = engine_->insert_record(record_to_insert);
  if (rc != RC::SUCCESS) {
    LOG_WARN("failed to insert new record during update. rc=%s", strrc(rc));
    return rc;
  }
...
RC Table::set_value_to_record(char *record_data, const Value &value, const FieldMeta *field)
{
  ...
  // 根据字段的实际类型（FieldMeta）进行分类处理
  switch (field->type()) {
    case AttrType::TEXT:
    case AttrType::CHARS: {
      // 针对字符串类型的特殊逻辑：处理截断和 \0 终止符
    ...
    default: {
      // 对于 INT, FLOAT 等其他类型，直接按长度进行物理拷贝
      // 注意：这里需要确保 value.length() 与 field->len() 一致，或已完成 Cast 转换
