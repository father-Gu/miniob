...
RC Db::drop_table(const char *table_name){
  ...
  if(table_name == nullptr || strlen(table_name) == 0){
    LOG_WARN("Invalid table name to drop.table name is null or empty.");
    return RC::INVALID_ARGUMENT;
  }

  // 查找表对象 (opened_tables_是哈希表,table_name映射到Table对象)
  ...
  // 调用表的drop方法，释放资源
 RC drop_rc = table->drop(path_.c_str());
  if (drop_rc != RC::SUCCESS) {
    LOG_ERROR("Failed to drop table files. table name=%s, rc=%d:%s", table_name, drop_rc, strrc(drop_rc));
    return drop_rc;
  }
  ...
  RC rc = sync();
  if (rc != RC::SUCCESS) {
    LOG_ERROR("Failed to sync db after drop table. table name=%s, rc=%d:%s", table_name, rc, strrc(rc));
    return rc;
  }
