...
RC DefaultConditionFilter::init(Table &table, const ConditionSqlNode &condition)
{
  ...
  if (condition.left_is_expr) {
    left_expression_ = condition.left_expr;
    left_is_expr_    = true;
  }

  if (condition.right_is_expr) {
    right_expression_ = condition.right_expr;
    right_is_expr_    = true;
  }

  // 如果两边都是表达式，直接保存指针，不需要调用init()
  if (left_is_expr_ && right_is_expr_) {
    table_   = &table;
    comp_op_ = condition.comp;
    return RC::SUCCESS;  // 直接返回
  }
...
bool DefaultConditionFilter::filter(const Record &rec) const
{
  ...
  if (left_is_expr_) {
    ...
    // 创建一个RowTuple来代表当前记录行
    RowTuple row_tuple;
    row_tuple.set_record(const_cast<Record *>(&rec));
    // 设置Tuple的schema（表的字段定义）
    row_tuple.set_schema(table_, table_->table_meta().field_metas());
    // 计算表达式的值
    RC rc = left_expression_->get_value(row_tuple, left_value);
    if (rc != RC::SUCCESS) {
      LOG_WARN("Failed to get value from left expression");
      return false;  // 表达式计算失败，这条记录不符合条件
    }
  } else if (left_.is_attr) {  // value
  ...
  if (right_is_expr_) {
    ...
    RC rc = right_expression_->get_value(row_tuple, right_value);
    if (rc != RC::SUCCESS) {
      LOG_WARN("Failed to get value from right expression");
      return false;
    }
