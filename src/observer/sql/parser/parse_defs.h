 ...
 * @brief 描述一个join语句
 * @ingroup SQLParser
 * @details 一个join语句由两部分组成，分别是join的表和join的条件。
 */
struct JoinSqlNode
{
  string                   relation;    ///< join的表(右边)
  vector<ConditionSqlNode> conditions;  ///< join条件(on)
};
...
struct SelectSqlNode
{
  ...
  vector<JoinSqlNode>            joins;        ///< join clauses
};
...
typedef vector<Value> RawTuple;//定义一个valve的数组，方便批处理
...
struct InsertSqlNode
{
  ...
  // vector<Value> values;      ///< 要插入的值 可能是多个字段的值
  vector<RawTuple> tuples;  ///< 要插入的行 升维 支持多行插入
};
