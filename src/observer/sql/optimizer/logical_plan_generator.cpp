...
{
  // 算子执行顺序：from -> where -> group by -> select
  // 算子树的构建顺序和执行顺序相反
  unique_ptr<LogicalOperator> *last_oper = nullptr;  // 指向“当前逻辑计划末尾节点”的指针（链式构建用）

  unique_ptr<LogicalOperator> table_oper(nullptr);  // 指向数据输入相关from子句的操作符（包括join）
  last_oper = &table_oper;                          // 初始时，逻辑计划的末尾节点就是table_oper
  unique_ptr<LogicalOperator> predicate_oper;       // 指向过滤相关where子句的操作符

  // 创建过滤逻辑计划（where子句），生成过滤算子predicate_oper
  ...
  // 创建数据输入逻辑计划（from子句），生成表扫描算子table_oper（如果有多个表，则生成连接算子）
  const vector<Table *> &tables      = select_stmt->tables();
  bool                   inner_join  = !select_stmt->join_stmts().empty();
  int                    table_index = 0;
  for (Table *table : tables) {
    // 为每个表创建表扫描算子
    unique_ptr<LogicalOperator> table_get_oper(new TableGetLogicalOperator(table, ReadWriteMode::READ_ONLY));
    if (table_oper == nullptr) {  // 第一个表，直接作为table_oper
      table_oper = std::move(table_get_oper);
    } else {  // 不是第一个表，生成连接算子，将当前table_oper和新表的扫描算子作为连接算子的子节点
      // 为inner join语句
      if (inner_join) {
        unique_ptr<JoinLogicalOperator> join_oper(new JoinLogicalOperator);
        join_oper->add_child(std::move(table_oper));
        join_oper->add_child(std::move(table_get_oper));
        // 处理连接条件（根据index找到当前连接对应的过滤语句）
        FilterStmt                 *join_filter_stmt = select_stmt->join_stmts()[table_index - 1]->filter_stmt();
        unique_ptr<LogicalOperator> join_predicate_oper;
        rc = create_plan(join_filter_stmt, join_predicate_oper);
        if (OB_FAIL(rc)) {
        ...
        table_oper = std::move(join_predicate_oper);
      } else {
        // 通过","隐式连接，无过滤条件
  ...
  // 将数据输入算子table_oper接到过滤算子predicate_oper上
  if (predicate_oper) {
    if (*last_oper) {
  ...
  // 创建group by逻辑计划（group by子句），生成group by算子
  ...
  // 将过滤算子predicate_oper接到group by算子上
  if (group_by_oper) {
    if (*last_oper) {
