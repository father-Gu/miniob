...
InsertStmt::InsertStmt(Table *table, const vector<RawTuple> tuples)
...
RC InsertStmt::create(Db *db, const InsertSqlNode &inserts, Stmt *&stmt)
{
  ...
  if (nullptr == db || nullptr == table_name || inserts.tuples.empty()) {
    LOG_WARN("invalid argument. db=%p, table_name=%p, value_line_num=%d",
  ...
  //对插入的每一条元素都解析
  //这里遍历了 inserts.tuples，支持 INSERT INTO table VALUES (...), (...); 这种多行插入语法。
